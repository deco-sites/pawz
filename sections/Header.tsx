...
const MenuItem = ({ title, url }: MenuProps) => (
  <li class="adjust-item flex items-center shrink-0 h-10 lg:px-0">
    <a
      href={url}
...
export default function Header({ logo, alt, menu }: Props) {
  return (
    ...
    >
      ...
      {menu && (
        <nav class="h-0 flex peer-checked:h-52 peer-checked:mt-6 lg:h-auto flex-col items-center justify-evenly overflow-hidden transition-all">
          <hr class="lg:hidden w-full border-gray-300 pb-2" />
          <div class="flex flex-col adjust-links items-center gap-4 ">
